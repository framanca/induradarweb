...
function headers(origin:string|null, type="application/json; charset=utf-8") {
  const h=new Headers({"Access-Control-Allow-Headers":"authorization, content-type","Access-Control-Allow-Methods":"GET, POST, OPTIONS","Cache-Control":"private, no-store, max-age=0","Content-Type":type,"Vary":"Origin","X-Content-Type-Options":"nosniff"});
...
function workbook(envelope:any){
  ...
  addSheet(wb,"Universo",profiles.map((raw)=>{const q=obj(raw),u=uById.get(String(q.company_id||""))||{},loc=obj(q.primary_location);return {Empresa:txt(q.name||u.company||u.name),"Razón social":txt(q.legal_name),"CIF/NIF":txt(q.tax_id),Tipo:typeLabel(q.company_type_code||u.company_type),Actividad:txt(q.activity_summary||u.activity),Provincia:txt(loc.province||u.province),Población:txt(loc.municipality||u.municipality),País:txt(loc.country||u.country),Web:txt(q.website||u.website),"Familias de producto":txt(q.product_family),"Áreas de aplicación":txt(q.application_area),Servicios:txt(q.service),Capacidades:txt(q.capability),Estado:txt(u.research_status||u.review_status||u.status)};}),[30,32,15,28,65,18,18,15,30,48,48,48,48,28]);
