...
declare
  ...
  p jsonb;
  c jsonb;
  cache private.report_web_payload_cache%rowtype;
  v_text text;
...
begin
  ...
  select * into cache
  from private.report_web_payload_cache w
  where w.report_version_id=r.id
    and w.report_reference=r.report_reference
    and w.source_payload_hash=coalesce(r.finalized_payload_hash,r.report_payload_hash)
    and coalesce((w.gate_snapshot->>'pass')::boolean,false)
    and coalesce(w.gate_snapshot->>'source','')='finalized_approved_report_payload_only'
    and jsonb_typeof(w.web_document#>'{payload}')='object'
  order by w.materialized_at desc
  limit 1;

  if found then
    p:=cache.web_document->'payload';
  else
  ...
  return jsonb_build_object(
    ...
    'source_payload_hash',coalesce(r.finalized_payload_hash,r.report_payload_hash),
