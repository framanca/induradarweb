...
create unique index if not exists feedback_portal_unique_idx
  on public.feedback(account_id,user_id,entity_type,entity_id,report_version_id)
  where user_id is not null and report_version_id is not null
    and entity_type in ('client_opportunity','company');

create table if not exists public.account_company_preferences (
...
alter table public.account_company_preferences enable row level security;

drop policy if exists account_company_preferences_member_select on public.account_company_preferences;
create policy account_company_preferences_member_select
on public.account_company_preferences
for select
to authenticated
using (public.is_account_member(account_id));

revoke all on public.account_company_preferences from anon;
grant select on public.account_company_preferences to authenticated;
grant all on public.account_company_preferences to service_role;

create index if not exists account_company_preferences_account_preference_idx
  on public.account_company_preferences(account_id,preference,company_id);
...
begin
  ...
  select rv.id, r.account_id, rv.report_payload, r.title
    into v_report_version_id, v_account_id, v_payload, v_title
  ...
  into v_companies
  from items i
  left join public.account_company_preferences p
...
begin
  ...
  if p_useful is null then
    raise exception 'useful_required' using errcode='22023';
  end if;
...
begin
  ...
  if p_preference='neutral' then
    delete from public.account_company_preferences
  ...
  insert into public.account_company_preferences(
...
create or replace function public.portal_request_report_update_v1(
  p_report_reference text,
  p_note text,
...
begin
  ...
  if v_note is null or length(v_note)<3 then
    raise exception 'update_note_required' using errcode='22023';
  end if;
  if length(v_note)>3000 then
    raise exception 'update_note_too_long' using errcode='22023';
  end if;

  select sr.id,sr.submission_id
    into v_request_id,v_submission_id
  from public.service_requests sr
  where sr.client_request_id=p_client_request_id
    and sr.created_by=v_uid
  limit 1;

  if v_request_id is not null then
    return jsonb_build_object(
      'pass',true,'reused',true,'request_id',v_request_id,'submission_id',v_submission_id
    );
  end if;

  select rv.id, r.account_id, r.request_id
    into v_report_version_id, v_account_id, v_source_request_id
  from public.report_versions rv
  join public.reports r on r.id=rv.report_id
  where rv.report_reference=upper(btrim(p_report_reference))
    and rv.status in ('ready','delivered')
  ...
  v_submission_id:=gen_random_uuid();
  v_request_key:='portal:report-update:'||upper(btrim(p_report_reference))||':'||
    substr(replace(p_client_request_id::text,'-',''),1,12);

  v_request_payload:=coalesce(v_source_request.form_payload,'{}'::jsonb);
  v_request_obj:=coalesce(v_request_payload->'request','{}'::jsonb)
    ||jsonb_build_object(
      'title','Actualización de '||upper(btrim(p_report_reference)),
      'cutoff_date',current_date::text
    );
  v_intake:=coalesce(v_request_payload->'intake_metadata','{}'::jsonb)||jsonb_build_object(
    'baseline_mode','explicit_report_update',
    'follow_up_kind','report_update',
    'source_report_reference',upper(btrim(p_report_reference)),
    'portal_update_note',v_note,
    'notification_email_inline',false
  );
  v_extensions:=coalesce(v_request_payload->'request_extensions','{}'::jsonb)||jsonb_build_object(
    'source_report_reference',upper(btrim(p_report_reference)),
  ...
  v_request_payload:=v_request_payload||jsonb_build_object(
    'submission_id',v_submission_id,
    'submitted_at',clock_timestamp(),
    'channel','web_form',
    'request',v_request_obj,
  ...
  insert into public.service_requests(
    ...
    v_account_id,v_source_request.seller_profile_id,v_submission_id,v_request_key,'web_form',
    'Actualización solicitada · '||upper(btrim(p_report_reference)),
    'received',coalesce(v_source_request.contract_version,'1.4.2'),v_request_payload,
    v_source_request.normalized_scope,current_date,v_source_request.neutral_output,
    false,clock_timestamp(),v_uid,p_client_request_id,null
  )
  returning id into v_request_id;
...
revoke all on function public.portal_get_report_interactions_v1(text) from public, anon;
revoke all on function public.portal_submit_feedback_v1(text,text,uuid,boolean,text) from public, anon;
revoke all on function public.portal_set_company_preference_v1(text,uuid,text,text) from public, anon;
revoke all on function public.portal_request_report_update_v1(text,text,uuid) from public, anon;

grant execute on function public.portal_get_report_interactions_v1(text) to authenticated, service_role;
grant execute on function public.portal_submit_feedback_v1(text,text,uuid,boolean,text) to authenticated, service_role;
grant execute on function public.portal_set_company_preference_v1(text,uuid,text,text) to authenticated, service_role;
grant execute on function public.portal_request_report_update_v1(text,text,uuid) to authenticated, service_role;
